"""R2 §八.4：以全新 run_id 重跑六类 + 授信（v7），绝不覆盖 v2…v6 历史产物。

v6 目录是在 g16 负面枚举归因（§四.D.9/D.10）、清单「单一恢复真相」佐证规则
（§四.B.6）与主题边界结构性证明（§四.A.8）**之前**的 ``set_enumeration`` /
``source_object_inventory`` 实现下落盘的，因此其对账产物已不是当前实现的输出。
v7 是首次由当前（含 A/B/C/D + 通用材料身份）实现落盘的真实 run，与 v6 并存，
供验收器读新产物；v6 及其之前的历史目录一律保留、不改写。
...
``explicit_cross_reference`` 复用 ``financial_notes`` v7 目录（显式引用事实在该 seed 中）。
...
# fixture 合成块/建库助手与 v5/v6 逐字一致（同一合成语料），此处只换 run_id，不改行为。
...
# 非 300750 合成 fixture 标识（与 v5/v6 同一合成语料，仅 run_id 不同）。
_COMPANY = "100001"
_DOC = "NDSD_DEMO"
_DOCV = "v-demo-001"
_SETV = "set-demo-1"
...
def _v7_run_id(category: str) -> str:
    return f"r2_sixcat_v7_{category}_{SUFFIX}"
...
def _gen_fixture() -> dict:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="non300750_fixture_v7_"))
    ...
    run_id = _v7_run_id("non_300750_fixture")
...
def main() -> None:
    ...
    # 1-4. 复用 v2 真实 seed manifest，重跑 v7。
    ...
    # 6. 非 300750 合成 fixture（v7）。
    fixture_summary = _gen_fixture()
    summaries.append({"category": "non_300750_fixture",
                      "run_id": _v7_run_id("non_300750_fixture"), **fixture_summary})

    print(json.dumps({"v7_artifacts": summaries}, ensure_ascii=False, indent=2))
