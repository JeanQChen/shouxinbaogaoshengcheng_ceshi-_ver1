"""R2 §六：**事前**冻结 v13 显式引用（结构性表引用绑定）验收样本清单（evaluation-only）。

与 v12 的差别（本轮只做三件窄范围收口，不做任何范围扩张）
------------------------------------------------------
1. **样本身份沿用 v12 事前冻结身份**：seed 七字段、锚点块身份、预期目标行、必须拒绝的
   P43 表5-6 全部逐字段读自 ``r2_xref_specimen_v12_20260916/specimen_manifest.json``
   （指纹 ``baa3a928…``），本轮不重新挑样本、不因结果改绑（§六）。
2. ``frozen_at`` 取**真实 UTC 时钟读数**（``datetime.now(timezone.utc)``），不使用固定值或
   未来时间；同时把清单/REDAME 的落盘时刻与文件 sha256 记进 ``specimen_file_facts.json``，
   供事后核对「清单先于 run 存在」。
3. 本轮**只重跑** ``company_subsidiaries.major_subsidiaries`` 正样本；其余五个真实样本
   原样绑定 v12 已冻结 run，标记 ``reused_from_previous_round``，不重跑、不调参（§六）。
4. 新增两条独立于生产实现的**事前断言**（由锚点原文的裸串/正则扫描推出，不 import 生产表
   结构模块）：同块标记之后的表题行唯一（``expected_same_block_caption_count == 1``），以及
   必须出现在受验对象表体内容里的真实业务行（企业名/持股比例/会计处理方法）。
...
PRIOR_SPECIMEN = RESULTS / "r2_xref_specimen_v12_20260916" / "specimen_manifest.json"
PRIOR_SPECIMEN_FINGERPRINT = (
    "baa3a92868b373cc5ce170b6c078603bf953b42f91beb31f88278da57d855e1c")
...
# 本轮**不重跑**的五个真实样本：绑定沿用 v12 已冻结 run（不重跑、不调参、不改绑）。
_REUSED_V12_RUNS = {
...
# 独立断言用的「业务行」判据：行内至少出现一个数字（金额/比例/序号），且不是表题行。
_DIGIT_RE = re.compile(r"\d")


def _captions_after(text: str, offset: int) -> list[dict]:
    """独立扫描：``offset`` 之后（含跨行）行首表题行（不 import 生产模块）。"""
    out: list[dict] = []
    off = 0
    for ln in text.splitlines(keepends=True):
        ...
        if _TABLE_TITLE_RE.match(ln):
            out.append({"line": ln.strip(), "start": s, "end": e})
    return out


def _business_lines(text: str, start: int) -> list[str]:
    """独立断言：表题行之后、含数字的非表题行（真实表体业务内容）。"""
    out: list[str] = []
    off = 0
    for ln in text.splitlines(keepends=True):
        s, e = off, off + len(ln)
        ...
        if s < start:
            continue
        n = ln.strip()
        if not n or _TABLE_TITLE_RE.match(ln):
            continue
...
def _load_prior() -> dict:
    if not PRIOR_SPECIMEN.exists():
        raise SystemExit(f"拒绝：缺少 v12 事前冻结清单 {PRIOR_SPECIMEN}")
    ...
    if got != PRIOR_SPECIMEN_FINGERPRINT:
        raise SystemExit(f"拒绝：v12 冻结清单指纹漂移 freeze={got}")
    if not spec.get("frozen_before_run", {}).get("all_run_dirs_absent"):
        raise SystemExit("拒绝：v12 清单不是 run 之前生成的")
    return spec
...
def _identity_from_prior(prior: dict) -> dict:
    ...
    if str(seed.get("evidence_id")) != POSITIVE_SEED_EVIDENCE_ID:
        raise SystemExit("拒绝：v12 冻结 seed 身份与既定正样本身份不一致")
    if (int(seed["page_number"]), int(seed["block_index"])) != POSITIVE_SEED_POSITION:
        raise SystemExit("拒绝：v12 冻结 seed 位置与既定现场不一致")
    if seed.get("aspect_id") != ASPECT_POSITIVE:
        raise SystemExit("拒绝：v12 冻结 seed aspect 与既定正样本 aspect 不一致")
    return dict(seed)


def _positive_sample(seed: dict, seed_facts: dict) -> dict:
    ...
    prior_anchor = json.loads(PRIOR_SPECIMEN.read_text(encoding="utf-8"))[
        "positive_sample"]["anchor_block"]
    if text_sha != prior_anchor["text_sha256"]:
        raise SystemExit("拒绝：锚点原文与 v12 冻结身份不一致（输入语料漂移）")
    ...
    return {
        ...
        "frozen_role": "本轮显式引用（结构性表引用绑定）能力的**正样本**；"
                       "身份逐字段取自 v12 事前冻结清单，本轮不重选、不改绑",
        ...
        "pre_declared_expectation": {
            ...
            "expected_same_block_caption_count": {
                ...
                "assertion": "标记之后同块内的表题行数 == 1，故同一 occurrence 下的真实表对象"
                             ...
                             "多于 1 时必须 fail-closed 而不是「恰好取到第一个」）",
            ...
            "expected_body_content_tokens": {
                ...
                "assertion": "受验对象绑定的表体行内容（空白归一化后）必须逐行包含这些真实业务"
                             "行；对象身份（object_id）必须随这些业务文本变化而变化，"
                             "仅空白/排版差异不得改变身份",
...
def _bound_run_binding() -> dict:
    binding = {
        ...
        "major_subsidiaries": f"r2_sixcat_{RUN_VERSION}_major_subsidiaries_{SUFFIX}",
    ...
    return {
        ...
        "rerun_this_round": {
            "company_subsidiaries.major_subsidiaries": {
                ...
                "why": "本轮显式引用结构性绑定的正样本（§六：只重跑该正样本 + 回归所需最小样本）",
            },
        },
        "reused_from_previous_round": {
            c: {"run_id": rid, "source_round": "v12",
                "why": "§一/§六：其余五个真实样本本轮不重跑、不调参、不改绑"}
...
def main() -> None:
    ...
    run_dirs = [f"r2_material_slice_r2_sixcat_{RUN_VERSION}_{c}_{SUFFIX}"
                for c in ("main_business", "core_competitiveness", "major_subsidiaries",
                          "financial_notes", "non_300750_fixture")]
    present = [d for d in run_dirs if (RESULTS / d).exists()]
    if present:
        raise SystemExit(f"拒绝：v13 run 目录在冻结前已存在 {present}")
    v13_glob = sorted(glob.glob(str(RESULTS / "*v13*")))
    if v13_glob:
        raise SystemExit(f"拒绝：冻结前已存在 v13 相关目录 {v13_glob}")

    frozen_at = _dt.datetime.now(_dt.timezone.utc)
    prior = _load_prior()
    seed_facts = _seed_manifest_facts()
    seed = _identity_from_prior(prior)
    positive = _positive_sample(seed, seed_facts)
    ...
    core = {
        ...
        "purpose": "事前冻结 v13 显式引用（结构性表引用绑定）验收样本：身份复用 v12 + 真实 UTC "
                   "冻结时刻 + 输入指纹 + 预期断言 + run 绑定",
        "authority": "R2 §六（Codex 独立验收结论作为本轮既定输入，不得重新论证/扩大范围）",
        "round": {
            "run_version": RUN_VERSION,
            "suffix": SUFFIX,
            "run_id_prefix": f"r2_sixcat_{RUN_VERSION}_",
            "rerun_scope": [ASPECT_POSITIVE],
            "not_rerun": sorted(_REUSED_V12_RUNS),
            "scope_note": "本轮收口只含三个生产 P1（引用 occurrence 身份 / 表对象资格 / 对象身份绑定"
                          "真实表体）＋两个提交阻塞项；不重跑、不调参其余五类样本",
        },
        "identity_source": {
            "prior_specimen": str(PRIOR_SPECIMEN).replace("\\", "/"),
            "prior_specimen_fingerprint": PRIOR_SPECIMEN_FINGERPRINT,
            "prior_specimen_frozen_at_label": prior.get("frozen_at"),
            "prior_label_disclosure": "v12 清单的 frozen_at 是**标称标签**（不是当时的真实时钟读数）；"
                                      "v13 改用真实 UTC 时钟读数并记录落盘时刻与文件哈希，"
                                      "以便事后核对「清单先于 run 存在」",
            "reuse_rule": "v13 的样本身份逐字段取自 v12 事前冻结清单；本轮不重新挑选样本、"
                          "不因运行结果改绑 run（§六）",
        },
        "frozen_before_run": {
            "run_dirs_present_at_freeze": [],
            "all_run_dirs_absent": True,
            "checked": ["六个 v13 run 目录", "evaluation/results/*v13*"],
        ...
        "post_run_rules": [
            ...
            "重跑后不得重跑或调参其余五类样本（绑定沿用 v12，逐条披露 reused_from_previous_round）",
            "若 input_identity 中任一 sha256 与冻结值不符，v13 验收 manifest 必须 fail-closed",
        ],
    }
    ...
    readme = [
        ...
        f"本轮只重跑 `{ASPECT_POSITIVE}`；其余五个样本绑定 v12 已冻结 run，"
        "标记 `reused_from_previous_round`，不重跑不调参。\n",
        "## 冻结时 v13 run 目录是否存在\n\n",
