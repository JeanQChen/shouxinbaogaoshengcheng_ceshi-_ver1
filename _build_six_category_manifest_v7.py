"""R2 §八.4：六类真实材料验收 manifest 生成器（v7：强化验收器读 v7 真实 run 目录）。

六类 v7 run 目录 → ``harness/six_category_acceptance.build_six_category_manifest`` 确定性聚合：
...
- ``explicit_cross_reference`` 复用 ``financial_notes`` v7 run 目录（显式引用事实在该 seed 中）。
...
只写 evaluation/results 产物（全新 v7 目录），不改冻结资产；v6 及更早目录一律保留。
"""
...
RUN_ID = "r2_sixcat_v7_20260916"
SUFFIX = "20260916"

# cid → v7 run 目录名（explicit_cross_reference 复用 financial_notes，其显式引用事实在财务附注 seed 中）。
CATEGORY_RUN_DIRS = {
    CATEGORY_MAIN_BUSINESS: f"r2_material_slice_r2_sixcat_v7_main_business_{SUFFIX}",
    CATEGORY_CORE_COMPETITIVENESS: f"r2_material_slice_r2_sixcat_v7_core_competitiveness_{SUFFIX}",
    CATEGORY_MAJOR_SUBSIDIARIES: f"r2_material_slice_r2_sixcat_v7_major_subsidiaries_{SUFFIX}",
    CATEGORY_FINANCIAL_NOTES: f"r2_material_slice_r2_sixcat_v7_financial_notes_{SUFFIX}",
    CATEGORY_EXPLICIT_CROSS_REFERENCE: f"r2_material_slice_r2_sixcat_v7_financial_notes_{SUFFIX}",
    CATEGORY_NON_300750_FIXTURE: f"r2_material_slice_r2_sixcat_v7_non_300750_fixture_{SUFFIX}",
}

OUT_DIR = RESULTS / "r2_six_category_acceptance_v7_20260916"
