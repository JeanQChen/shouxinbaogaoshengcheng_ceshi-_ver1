"""R2 §12 修复 九：以新 run_id 重生成真实验收产物（v4，绝不覆盖历史 v2/v3 目录）。
...
1-4. ``main_business`` / ``core_competitiveness`` / ``major_subsidiaries`` / ``financial_notes``
    —— 含修复 B（源对象清单）+ 修复 C（跨页续表同一张表证明）的 material build；
5. ``credit``（授信语义）—— p0 credit seed 复用，供修复 E 预览派生（授信双轴）；
6. ``non_300750_fixture`` —— 合成 fixture（无公司硬编码），独立持久化；
7. 六类强门 manifest —— 由 ``harness.six_category_acceptance``（修复 D 强验收器）对 v4 目录聚合；
8. 授信双轴 v2 预览 —— 由 ``harness.credit_fact_extraction``（修复 E）从 v4 credit 材料派生。

``explicit_cross_reference`` 复用 ``financial_notes`` v4 目录（显式引用事实在财务附注 seed 中）。
...
# v2 → v4 run_id 映射（4 个真实类别 + credit；explicit_reference 复用 financial_notes）。
_CATEGORY_V2_DIRS = {
...
def _v4_run_id(category: str) -> str:
    return f"r2_sixcat_v4_{category}_{SUFFIX}"
...
def _gen_fixture() -> dict:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="non300750_fixture_v4_"))
    ...
    run_id = _v4_run_id("non_300750_fixture")
...
def main() -> None:
    ...
    # 1-4. 复用 v2 真实 seed manifest，重跑 v4。
    ...
    # 6. 非 300750 合成 fixture（v4）。
    fixture_summary = _gen_fixture()
    summaries.append({"category": "non_300750_fixture",
                      "run_id": _v4_run_id("non_300750_fixture"), **fixture_summary})

    print(json.dumps({"v4_artifacts": summaries}, ensure_ascii=False, indent=2))
