...
> **当前角色：CONFIRMED FINANCIAL POLICY。** 本文继续约束 Financial V2 公式、代理值、缺失值和口径；它不授权重新实施 A6/A7，也不定义 P3R/P4R 章节结构。
> 生成时间：2026-09-07
> 历史状态：**2026-09-07 已完成本轮业务口径确认；当时据此进入 A6，A6/A7 现已关闭。**
...
- **舍入（已确认技术默认）**：Python 计算和数据库权威值保留完整 `Decimal` 精度；仅展示层舍入。
  普通比率展示 2 位小数，百分比展示 2 位小数，统一使用 `ROUND_HALF_UP`。展示值不得反向写入、
  覆盖或参与后续计算；公式结果必须同时保留未舍入权威值和舍入规则版本。
