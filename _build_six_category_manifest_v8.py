"""R2 §八.4：六类真实材料验收 manifest 生成器（v8 = 本轮定稿轮，强化验收器读 v8 真实 run 目录）。

六类 v8 run 目录 → ``harness/six_category_acceptance.build_six_category_manifest`` 确定性聚合：
...
- ``explicit_cross_reference`` 复用 ``financial_notes`` v8 run 目录（显式引用事实在该 seed 中）。
...
只写 evaluation/results 产物（全新 v8 目录），不改冻结资产；v6/v7 及更早目录一律保留。
"""
...
RUN_ID = "r2_sixcat_v8_20260916"
SUFFIX = "20260916"

# cid → v8 run 目录名（explicit_cross_reference 复用 financial_notes，其显式引用事实在财务附注 seed 中）。
CATEGORY_RUN_DIRS = {
    CATEGORY_MAIN_BUSINESS: f"r2_material_slice_r2_sixcat_v8_main_business_{SUFFIX}",
    CATEGORY_CORE_COMPETITIVENESS: f"r2_material_slice_r2_sixcat_v8_core_competitiveness_{SUFFIX}",
    CATEGORY_MAJOR_SUBSIDIARIES: f"r2_material_slice_r2_sixcat_v8_major_subsidiaries_{SUFFIX}",
    CATEGORY_FINANCIAL_NOTES: f"r2_material_slice_r2_sixcat_v8_financial_notes_{SUFFIX}",
    CATEGORY_EXPLICIT_CROSS_REFERENCE: f"r2_material_slice_r2_sixcat_v8_financial_notes_{SUFFIX}",
    CATEGORY_NON_300750_FIXTURE: f"r2_material_slice_r2_sixcat_v8_non_300750_fixture_{SUFFIX}",
}

OUT_DIR = RESULTS / "r2_six_category_acceptance_v8_20260916"
