"""R2 §12 修复 八：以新 run_id 重生成 8 个真实产物（v3，绝不覆盖历史 v2 目录）。
...
1. ``main_business`` —— 修复 A：主题边界块级真实边界（安全生产/在建工程/未来规划/行业 被
   ``outside_boundary_sentinel`` 停，绝不采纳进材料）；
2. ``core_competitiveness`` —— 每版本独立枚举（跨 document_version 不合并伪造完整集）；
3. ``major_subsidiaries`` —— 每版本独立枚举；
4. ``financial_notes`` —— 修复 D：跨块/跨页续（p166→p167+ 续行块因 section_path unreliable
   不再被误停）+ 显式引用解析（``详见 24、所有权或使用权受到限制的资产`` 经
   ``find_by_section_reference`` 确定性解析）；
5. ``explicit_reference`` —— 与 financial_notes 同 run 目录（显式引用事实在财务附注 seed 中）；
6. ``non_300750_fixture`` —— 合成 fixture（无公司硬编码），独立持久化；
7. 六类强门 manifest —— 由 ``_build_six_category_manifest.py``（修复 B/C 强验收器）另跑；
8. 授信双轴 v2 预览 —— 由 ``_gen_credit_dual_axis_v2_preview.py``（修复 E）另跑。
...
# v2 → v3 run_id 映射（4 个真实类别；explicit_reference 复用 financial_notes）。
_CATEGORY_V2_DIRS = {
...
def _v3_run_id(category: str) -> str:
    return f"r2_sixcat_v3_{category}_{SUFFIX}"
...
def _gen_fixture() -> dict:
    ...
    tmp = Path(tempfile.mkdtemp(prefix="non300750_fixture_v3_"))
    ...
    run_id = _v3_run_id("non_300750_fixture")
...
def main() -> None:
    ...
    # 1-4. 复用 v2 真实 seed manifest，重跑 v3。
    ...
    # 5. 非 300750 合成 fixture（v3）。
    fixture_summary = _gen_fixture()
    summaries.append({"category": "non_300750_fixture",
                      "run_id": _v3_run_id("non_300750_fixture"), **fixture_summary})

    print(json.dumps({"v3_artifacts": summaries}, ensure_ascii=False, indent=2))
